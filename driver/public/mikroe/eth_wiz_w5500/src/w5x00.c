...
uint16_t w5x00_write(uint16_t addr, const uint8_t *buf, uint16_t len)
{
  ...
  if (chip == W5x00_W5100) {
    ...
    if (addr < 0x100) {
      ...
      cmd[0] = addr >> 8;
      cmd[1] = addr & 0xFF;
#if W5x00_ETHERNET_LARGE_BUFFERS_ENABLE && W5x00_MAX_SOCK_NUM <= 1
      cmd[2] = 0x14;                       // 16K buffers
#elif W5x00_ETHERNET_LARGE_BUFFERS_ENABLE && W5x00_MAX_SOCK_NUM <= 2
      cmd[2] = ((addr >> 8) & 0x20) | 0x14; // 8K buffers
#elif W5x00_ETHERNET_LARGE_BUFFERS_ENABLE && W5x00_MAX_SOCK_NUM <= 4
