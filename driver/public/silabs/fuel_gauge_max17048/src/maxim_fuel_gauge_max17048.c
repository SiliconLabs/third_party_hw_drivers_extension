...
// Global variables
static sl_i2cspm_t  *max17048_i2cspm_instance = NULL;
...
static uint32_t     max17048_rcomp_update_interval =
  MAX17048_CONFIG_RCOMP_UPDATE_INTERVAL_MS;
...
sl_status_t max17048_init(sl_i2cspm_t *i2cspm)
{
  ...
  TEMPDRV_Init();
  status =
    max17048_register_temperature_callback(
      ((max17048_temp_callback_t)((int32_t)TEMPDRV_GetTemp)));
  if (status != SL_STATUS_OK) {
    return status;
  }
...
{
  return (uint32_t)((max17048_vreset_tracking >>
...
{
  return (uint32_t)(max17048_actthr_tracking * MAX17048_ACTTHR_RESOLUTION
                    / 1000);
}
