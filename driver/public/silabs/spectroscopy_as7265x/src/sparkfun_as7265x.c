...
// Device type array.
static const uint8_t device_type[3] = { SPARKFUN_AS72653_UV,
                                        SPARKFUN_AS72652_VISIBLE,
                                        SPARKFUN_AS72651_NIR };
...
sl_status_t sparkfun_as7265x_init(sl_i2cspm_t *i2cspm)
{
  ...
  sc |= sparkfun_as7265x_set_bulb_current(
  ...
  sc |= sparkfun_as7265x_disable_bulb(SPARKFUN_AS7265x_LED_WHITE);
  sc |= sparkfun_as7265x_disable_bulb(SPARKFUN_AS7265x_LED_IR);
  sc |= sparkfun_as7265x_disable_bulb(SPARKFUN_AS7265x_LED_UV);

  // 50 * 2.8ms = 140ms. 0 to 255 is valid.
  sc |= sparkfun_as7265x_set_integration_cycles(49);

  // Set to 8mA (maximum)
  sc |= sparkfun_as7265x_set_indicator_current(
    SPARKFUN_AS7265X_INDICATOR_CURRENT_LIMIT_8MA);
  sc |= sparkfun_as7265x_enable_indicator();

  // Set gain to 64x
  sc |= sparkfun_as7265x_set_gain(SPARKFUN_AS7265X_GAIN_64X);

  /* One-shot reading of VBGYOR. If you use Mode 2 or 3 then integration time
   * is double between readings.
   */
  sc |= sparkfun_as7265x_set_measurement_mode(
    SPARKFUN_AS7265X_MEASUREMENT_MODE_6CHAN_CONTINUOUS);

  sc |= sparkfun_as7265x_enable_interrupt();
...
sl_status_t sparkfun_as7265x_take_measurements(void)
{
  ...
  uint32_t start_time = sparkfun_as7265x_get_ms();

  // Wait for data to be ready
  while (1) {
    sc = sparkfun_as7265x_data_available(&is_data_ready);
    if (sc != SL_STATUS_OK) {
      return SL_STATUS_FAIL;
    }
    ...
    if ((sparkfun_as7265x_get_ms() - start_time)
        > max_wait_time) {
...
sl_status_t sparkfun_as7265x_take_measurements_with_bulb(void)
{
  ...
  sc |= sparkfun_as7265x_enable_bulb(SPARKFUN_AS7265x_LED_WHITE);
  sc |= sparkfun_as7265x_enable_bulb(SPARKFUN_AS7265x_LED_IR);
  sc |= sparkfun_as7265x_enable_bulb(SPARKFUN_AS7265x_LED_UV);

  sc |= sparkfun_as7265x_take_measurements();
  ...
  sparkfun_as7265x_color_data_t *color_data)
{
  ...
  uint8_t i = 0;
  uint8_t j = 0;
  uint16_t *color_channel =
    &color_data->channel[SPARKFUN_AS7265x_COLOR_CHANNEL_A];
...
sl_status_t sparkfun_as7265x_get_channel(
  ...
  uint16_t *color_data)
{
  if ((!color_data) || (channel >= SPARKFUN_AS7265x_COLOR_CHANNEL_MAX)) {
    return SL_STATUS_INVALID_PARAMETER;
  }
  ...
  sparkfun_as7265x_calibrated_data_t *cal_val)
{
  ...
  uint8_t i = 0;
  uint8_t j = 0;
  float *calib_data_channel =
    &cal_val->channel[SPARKFUN_AS7265x_COLOR_CHANNEL_A];
...
sl_status_t sparkfun_as7265x_get_calibrated_value(
  ...
  float *cal_val)
{
  if ((!cal_val) || (channel >= SPARKFUN_AS7265x_COLOR_CHANNEL_MAX)) {
    return SL_STATUS_INVALID_PARAMETER;
  }
...
sl_status_t sparkfun_as7265x_enable_bulb(sparkfun_as7265x_led_type_t led_type)
{
  ...
  sc |= select_device((sparkfun_as7265x_device_type_t)led_type);
...
sl_status_t sparkfun_as7265x_enable_indicator(void)
{
  ...
  sc |= select_device(SPARKFUN_AS72651_NIR);
...
static sl_status_t write_virtual_register(uint8_t virtual_addr, uint8_t data)
{
  ...
  // Wait for WRITE register to be empty
  start_time = sparkfun_as7265x_get_ms();
  while (1)
  {
    if ((sparkfun_as7265x_get_ms() - start_time)
        > max_wait_time) {
