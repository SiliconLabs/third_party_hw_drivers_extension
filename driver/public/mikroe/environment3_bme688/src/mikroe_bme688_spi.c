...
sl_status_t mikroe_environment3_init(SPIDRV_Handle_t spidrv_inst)
{
  ...
  environment3_cfg.cs = hal_gpio_pin_name(spidrv_inst->portCs,
                                          spidrv_inst->pinCs);
  GPIO_PinModeSet(spidrv_inst->portCs, spidrv_inst->pinCs, gpioModePushPull, 1);
  environment3_cfg.drv_sel = ENVIRONMENT3_DRV_SEL_SPI;

  environment3.spi.handle = spidrv_inst;
