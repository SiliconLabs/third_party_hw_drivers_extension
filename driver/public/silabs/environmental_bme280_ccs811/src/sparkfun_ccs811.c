...
// Local prototypes
static uint32_t sparkfun_ccs811_erase_application(sl_i2cspm_t *i2cspm);
static uint32_t sparkfun_ccs811_verify_application(sl_i2cspm_t *i2cspm,
                                                   bool *appValid);
static uint32_t sparkfun_ccs811_program_firmware(sl_i2cspm_t *i2cspm,
                                                 uint8_t buffer[]);
static uint32_t sparkfun_ccs811_set_app_start(sl_i2cspm_t *i2cspm);
...
{
  if (wake) {
    GPIO_PinOutClear(SPARKFUN_CCS811_WAKE_PORT, SPARKFUN_CCS811_WAKE_PIN);
    sl_sleeptimer_delay_millisecond(1);
  } else {
    GPIO_PinOutSet(SPARKFUN_CCS811_WAKE_PORT, SPARKFUN_CCS811_WAKE_PIN);
    sl_sleeptimer_delay_millisecond(1);
  }
  return SL_STATUS_OK;
}
...
sl_status_t sparkfun_ccs811_init(sl_i2cspm_t *i2cspm)
{
  ...
  GPIO_PinModeSet(SPARKFUN_CCS811_WAKE_PORT,
  ...
  status = sparkfun_ccs811_get_hardware_id(i2cspm, &id);
  if ((status != SL_STATUS_OK) || (id != SPARKFUN_CCS811_HW_ID)) {
    return SL_STATUS_INITIALIZATION;
  }

  /* Switch from boot mode to application mode */
  status = sparkfun_ccs811_start_application(i2cspm);
  if (status != SL_STATUS_OK) {
    return status;
  }

  status = sparkfun_ccs811_set_measure_mode(i2cspm,
 ...
 ******************************************************************************/
sl_status_t sparkfun_ccs811_deinit(sl_i2cspm_t *i2cspm)
{
  /* Disable the sensor  */
  (void)i2cspm;
 ...
 ******************************************************************************/
sl_status_t sparkfun_ccs811_get_hardware_id(sl_i2cspm_t *i2cspm,
                                            uint8_t *hardwareID)
{
  sl_status_t result;

  result = sparkfun_ccs811_read_mailbox(i2cspm,
                                        SPARKFUN_CCS811_ADDR_HW_ID,
                                        1,
                                        hardwareID);
...
bool sparkfun_ccs811_is_data_available(sl_i2cspm_t *i2cspm)
{
  ...
  status = sparkfun_ccs811_get_status(i2cspm, &reg);
...
sl_status_t sparkfun_ccs811_start_application(sl_i2cspm_t *i2cspm)
{
  ...
  /* Issue APP_START */
  result = sparkfun_ccs811_set_app_start(i2cspm);
                                         ...
                                         uint8_t *data)
{
  ...
  seq.addr = SPARKFUN_CCS811_DEFAULT_ADDR << 1;
                                            ...
                                            uint16_t *tvoc)
{
  ...
  *tvoc = 0;
  ...
  i2c_write_data[0] = SPARKFUN_CCS811_ADDR_ALG_RESULT_DATA;
                                         ...
                                         uint16_t *rawData)
{
  ...
  *rawData = 0;
  ...
  /* Read four bytes from the SPARKFUN_CCS811_ADDR_RAW_DATA mailbox register */
  i2c_write_data[0] = SPARKFUN_CCS811_ADDR_RAW_DATA;
...
static uint32_t sparkfun_ccs811_set_app_start(sl_i2cspm_t *i2cspm)
{
  ...
  i2c_write_data[0] = SPARKFUN_CCS811_ADDR_APP_START;
                                                  ...
                                                  uint16_t *fw_version)
{
  sl_status_t status;
  uint8_t buffer[2];
  status = sparkfun_ccs811_read_mailbox(i2cspm,
                                        SPARKFUN_CCS811_ADDR_FW_APP_VERSION,
                                        2,
                                        buffer);
  if (status != SL_STATUS_OK) {
    return status;
                                            ...
                                            uint32_t length)
{
  ...
  if ((reg & 0x80) == 0x80) {
    ...
    status = sparkfun_ccs811_software_reset(i2cspm);
    if ((status & 0x80) != 0x00) {
  ...
  if ((reg & 0x10) == 0x10) {
    ...
    status = sparkfun_ccs811_erase_application(i2cspm);
  ...
  for ( i = 0; i < length; i += 8 ) {
    ...
    status = sparkfun_ccs811_program_firmware(i2cspm, buffer);
    if (status != SL_STATUS_OK) {
      return SL_STATUS_FAIL;
    }
  }
  ...
  /** Verify APP                                                          **/
  /*************************************************************************/
  status = sparkfun_ccs811_verify_application(i2cspm, &valid);
                                                   ...
                                                   bool *appValid)
{
  ...
  *appValid = false;
  ...
  i2c_write_data[0] = SPARKFUN_CCS811_ADDR_FW_VERIFY;
