...
sl_status_t triacdrv_initPRS(ACMP_TypeDef *acmp,
                             uint32_t acmpChannel,
                             TIMER_TypeDef *tmr,
                             uint32_t tmrChannel);
...
sl_status_t triacdrv_init(const TRIACDRV_Init_TypeDef *init)
{
   ...
   * channel.
   */
  GPIO_PinModeSet(init->timerPrsOutputPort,
                  init->timerPrsOutputPin,
                  gpioModePushPull,
                  0);
  PRS_GpioOutputLocation(init->timerPrsBaseChannel,
                         init->timerPrsOutputLoc);

  // Initialize the selected ACMP with the specified input and threshold
  triacdrv_initACMP(init->acmp,
                    init->acmpInput,
                    init->inputWave,
                    init->avdd,
                    init->zeroThreshold);
                        ...
                        bool initOn)
{
   ...
   *
   * (TIMER clock freq / TIMER prescaler) * (gate enable pulse width in us)
   * ----------------------------------------------------------------------
   *                                1,000,000
   *
...
#if TRIACDRV_DEBUG_PULSE_RISE_TIMER_OUT
  GPIO_PinModeSet(TRIACDRV_DEBUG_PULSE_RISE_TIMER_OUT_PORT,
                  TRIACDRV_DEBUG_PULSE_RISE_TIMER_OUT_PIN,
                  gpioModePushPull,
                  0);
...
#if TRIACDRV_DEBUG_PULSE_FALL_TIMER_OUT
  GPIO_PinModeSet(TRIACDRV_DEBUG_PULSE_FALL_TIMER_OUT_PORT,
                  TRIACDRV_DEBUG_PULSE_FALL_TIMER_OUT_PIN,
                  gpioModePushPull,
                  1);
...
#if TRIACDRV_DEBUG_PRSACMP
  GPIO_PinModeSet(TRIACDRV_DEBUG_PRSACMP_PORT,
                  TRIACDRV_DEBUG_PRSACMP_PIN,
                  gpioModePushPull,
                  0);
...
#if TRIACDRV_DEBUG_PULSE_RISE_PRS_OUT
  GPIO_PinModeSet(TRIACDRV_DEBUG_PULSE_RISE_PRS_OUT_PORT,
...
#if TRIACDRV_DEBUG_PULSE_FALL_PRS_OUT
  GPIO_PinModeSet(TRIACDRV_DEBUG_PULSE_FALL_PRS_OUT_PORT,
                  TRIACDRV_DEBUG_PULSE_FALL_PRS_OUT_PIN,
                  gpioModePushPull,
                  1);

  // Route CC2 PRS output to debug pin
  PRS_GpioOutputLocation(tmrChannel + 1,
                         TRIACDRV_DEBUG_PULSE_FALL_PRS_OUT_LOC);
#endif
...
#if TRIACDRV_DEBUG_ACMPOUT
  GPIO_PinModeSet(TRIACDRV_DEBUG_ACMPOUT_PORT,
                  TRIACDRV_DEBUG_ACMPOUT_PIN,
                  gpioModePushPull,
                  0);
...
#endif /* TRIACDRV_DISABLE_HW_RESOURCE_CHECKING */
{
   ...
   * for the driver.
   */
  if ((avdd > TRIACDRV_MAX_AVDD) || (avdd < TRIACDRV_MIN_AVDD)) {
    return SL_STATUS_INVALID_RANGE;
  }
